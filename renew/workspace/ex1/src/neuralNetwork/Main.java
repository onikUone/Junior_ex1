...
public class Main {
	...
	//順方向計算メソッド
	public static OutputNeuron[] forward(InputNeuron input[], HiddenNeuron inter[], OutputNeuron out[]){
...
//		String borderPath = "C:\\Users\\Yuichi Omozaki\\IDrive\\Junior_ex2\\eclipse_workspace\\Junior_ex2\\src\\border.dat";	//Windows(研究室環境)
		...
		HiddenNeuron inter[] = new HiddenNeuron[interNumber];
		for(int i=0; i<inter.length; i++){
			inter[i] = new HiddenNeuron(inputNumber, Math.random(), Math.random(), preEta, preAlpha);	//コンストラクタには前層の個数を指定 = weightの個数を決定する
		}
